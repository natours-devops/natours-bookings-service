...
exports.getCheckoutSession = catchAsync(async (req, res, next) => {
  ...
  const { data } = await axios.get(`${TOUR_SERVICE_URL}/api/v1/tours/${req.params.tourId}`);
...
exports.confirmBooking = catchAsync(async (req, res, next) => {
  ...
  if (!tour || !price)
    return next(new AppError('Tour and price are required to confirm a booking', 400));
...
exports.getMyBookings = catchAsync(async (req, res, next) => {
  ...
  res.status(200).json({ status: 'success', results: bookings.length, data: { data: bookings } });
});
